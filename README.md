...
Mdict 支持两种显示模式，根据模板中是否存在指定容器自动选择：

### 嵌入式模式（Embedded Mode）

当模板中存在 `<div id="mdict-panel"></div>` 容器时，Mdict 会自动检测并渲染到该容器内。
...
**容器检测**：
```javascript
// 初始化时自动检测（默认检测 #mdict-panel）
MD.UI.detectContainer('mdict-panel');  // 返回 true 表示检测到容器

// 检查当前模式
MD.UI.isEmbedded();  // 返回 true 表示嵌入式模式
```

### 弹窗模式（Modal Mode）

当模板中不存在指定容器时，Mdict 会创建居中浮层显示查词结果。
...
**模式判断**：
```javascript
MD.UI.isModal();  // 返回 true 表示弹窗模式
```

### 延迟设置容器

支持容器后加载的场景（例如容器由 JavaScript 动态创建）：

```javascript
// 在容器创建后调用
MD.UI.setContainer('mdict-panel');  // 切换到嵌入式模式
```

### ListenSpeak 集成方式

ListenSpeak 的 `back.html` 模板预置了 `<div id="mdict-panel"></div>` 容器，用户通过以下步骤启用字典功能：

1. 安装 Anki_Mdict 插件
2. 导入 MDX 字典并配置
3. 使用"模板注入"功能将 Mdict 脚本注入到 ListenSpeak 笔记类型
4. 字典面板自动渲染到 `#mdict-panel` 容器内（嵌入式模式）

**技术细节**：
- 容器 ID：`mdict-panel`（固定）
- 检测时机：`MD.UI.init()` 或首次调用 `MD.UI.getMode()`
- 模式切换：容器存在 → embedded，容器不存在 → modal
