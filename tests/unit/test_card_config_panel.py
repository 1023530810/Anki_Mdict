...
def test_get_dictionaries_enabled_only_uses_managed_ids() -> None:
    content = _read_media_file("_mdict_main.js")
    assert "enabledOnly" in content
    assert re.search(r"tokenizers\[opts\.language\]\.dictionaryIds", content)
...
def test_lookup_language_threads_through_flow() -> None:
    ...
    assert re.search(
        r"lookupFromToken\(word,\s*null,\s*prefixHtml,\s*language,\s*element\)",
        main_content,
    )
    assert re.search(
...
def test_lookup_scrolls_and_syncs_dictionary_select() -> None:
    ...
    assert re.search(r"selectDictionary\(result\.dictionaryId", ui_content)
